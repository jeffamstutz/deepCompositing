...
{
  CUDA_CALL(
      Memcpy(host.data(), dev, host.size() * sizeof(T), cudaMemcpyDefault));
}
...
{
  if (fbSize == newSize) /* already same size - just skip */
    return;
  ...
  if (compactedFragmentLists)
    CUDA_CALL(Free(compactedFragmentLists));
  if (fixedSizeFragmentLists)
    CUDA_CALL(Free(fixedSizeFragmentLists));
  if (lowBitCounters)
    CUDA_CALL(Free(lowBitCounters));
  if (fullIntCounters)
    CUDA_CALL(Free(fullIntCounters));
  ...
  */
  int numIntsForCountersArray =
      std::max(numPixelsPadded, numRanks() * computeNumPixelsOnThisRank());
  CUDA_CALL(
      Malloc(&fullIntCounters, numIntsForCountersArray * sizeof(uint32_t)));
  CUDA_CALL(
      Memset(fullIntCounters, 0, numIntsForCountersArray * sizeof(uint32_t)));
  CUDA_CALL(Malloc(&lowBitCounters, compactCounterBytes * sizeof(uint8_t)));
  ...
  CUDA_CALL(Malloc(&fixedSizeFragmentLists, fragArraySize));
  CUDA_CALL(Malloc(&compactedFragmentLists, fragArraySize));
...
int Compositor::affinitizeGPU()
{
  ...
  MPI_CALL(Allgather(sendBuf.data(),
      sendBuf.size(),
      MPI_CHAR,
      recvBuf.data(),
      /*yes, SENDbuf here*/ sendBuf.size(),
      MPI_CHAR,
      comm));
...
void Compositor::save(const std::string &fileName)
{
  ...
  uint32_t numCompactedFragments = download(fullIntCounters, numPixelsOrg - 1);
  ...
  out.write((char *)h_fullIntCounters.data(),
      numPixelsOrg * sizeof(fullIntCounters[0]));
  ...
  download(h_compactedFragmentLists, compactedFragmentLists);
  out.write((char *)h_compactedFragmentLists.data(),
      numCompactedFragments * sizeof(compactedFragmentLists[0]));
  std::cout << "SUCCESSFULLY DUMPED FRAME BUFFER, fbsize=" << fbSize.x << "x"
            << fbSize.y << " numFrags=" << numCompactedFragments << std::endl;
}
...
SavedFrags SavedFrags::load(const std::string &fileName)
{
  ...
  if (!in.good())
    throw std::runtime_error(
        "cloud not load deep frame buffer from " + fileName);
  ...
  in.read((char *)h_counters.data(), // result.counters,
      result.fbSize.x * result.fbSize.y * sizeof(*result.counters));
  CUDA_CALL(Memcpy(result.counters,
      h_counters.data(),
      result.fbSize.x * result.fbSize.y * sizeof(*result.counters),
      cudaMemcpyDefault));
  ...
  CUDA_CALL(Malloc(&result.fragments, numFrags * sizeof(*result.fragments)));

  std::vector<Fragment> h_frags(numFrags);
  in.read((char *)h_frags.data(), numFrags * sizeof(*result.fragments));
  CUDA_CALL(Memcpy(result.fragments,
      h_frags.data(),
...
__global__ void compactFragmentsKernel(Fragment *compacted,
    uint32_t *offsets,
    ...
    int numPixelsPadded)
{
  ...
  if (pixelID >= numPixelsPadded)
    return;

  uint32_t end = offsets[pixelID];
  uint32_t begin = (pixelID == 0) ? 0 : offsets[pixelID - 1];
  int count = end - begin;
...
__global__ void compositeKernel(uint32_t *compositedColor,
    ...
    int /*mpi group size*/ size)
{
  ...
  if (pixelIdx >= numPixelsOnThisRank)
    return;
  ...
  while (1) {
    ...
    if (nextClosestFragment == nullptr
        || float(nextClosestFragment->z) >= 1e20f)
      break;

    float4 fragColor = nextClosestFragment->getRGBA();
    color = color + (1.f - alpha) * (const float3 &)fragColor;
...
inline std::ostream &operator<<(std::ostream &o, const Fragment &frag)
{
  o << "{z=" << float(frag.z) << ",rgb=(" << (float)frag.r << ","
    << (float)frag.g << "," << (float)frag.b << ")"
    << ",a=" << frag.getRGBA().w << "}";
  return o;
}
...
Compositor::Compositor(int maxFragsPerPixel, MPI_Comm comm)
    : comm(comm),
...
DeviceInterface Compositor::prepare()
{
  if (fbSize.x <= 0 || fbSize.y <= 0)
    throw std::runtime_error(
        "invalid or un-specified frame buffer size in dc library..."
  ...
  CUDA_CALL(Memset(fullIntCounters,
      0,
...
void Compositor::finish(uint32_t *whereToWriteFinalPixels)
{
  ...
  CUDA_SYNC_CHECK();
  const uint32_t numPixelsOnThisRank = pixelEnd(rank) - pixelBegin(rank);
  ...
  // ------------------------------------------------------------------
  {
    ...
    CUDA_CALL(Memset(fullIntCounters + numPixelsOrg,
        0,
    ...
    thrust::inclusive_scan(thrust::device,
        ...
        thrust::device_pointer_cast<uint32_t>(
            fullIntCounters + numPixelsPadded),
        thrust::device_pointer_cast<uint32_t>(fullIntCounters));
#endif
    CUDA_SYNC_CHECK();
    ...
    compactFragmentsKernel<<<numBlocks, blockSize>>>(compactedFragmentLists,
        fullIntCounters,
        fixedSizeFragmentLists,
        maxFragsPerPixel,
        numPixelsPadded);
  ...
  for (int node = 0; node < size; node++) {
    ...
    numFragmentsTo[node] =
        ...
        - (node_pixel_begin ? download(fullIntCounters, node_pixel_begin - 1)
                            : 0);
    numBytesTo[node] = numFragmentsTo[node] * sizeof(Fragment);
  ...
  for (int node = 0; node < size; node++) {
    ...
    numCounterBytesTo[node] =
        (pixelEnd(node) - pixelBegin(node)) / numCountersPerByte;
  ...
  CUDA_CALL(Malloc(&recvCompactCounters, sumCounterBytesReceiving));
...
#endif
  MPI_CALL(Alltoallv(lowBitCounters,
      numCounterBytesTo.data(),
      ofsCounterBytesTo.data(),
      MPI_BYTE,
      recvCompactCounters,
      numCounterBytesFrom.data(),
      ofsCounterBytesFrom.data(),
      MPI_BYTE,
      comm));
  CUDA_SYNC_CHECK();
  ...
  // ------------------------------------------------------------------
  {
    ...
    uncompressCountersKernel<<<numBlocks, blockSize>>>(fullIntCounters,
        recvCompactCounters,
  ...
  thrust::inclusive_scan(thrust::device,
      thrust::device_pointer_cast<uint32_t>(fullIntCounters),
      thrust::device_pointer_cast<uint32_t>(
          fullIntCounters + numCountersReceived),
      thrust::device_pointer_cast<uint32_t>(fullIntCounters));
#endif
  CUDA_SYNC_CHECK();
  ...
  for (int node = 0; node < size; node++) {
    ...
    numFragmentsFrom[node] =
        ...
        - (node_comp_begin ? download(fullIntCounters, node_comp_begin - 1)
                           : 0);
    numBytesFrom[node] = numFragmentsFrom[node] * sizeof(Fragment);
  ...
  MPI_CALL(Alltoallv(compactedFragmentLists,
      numBytesTo.data(),
      ofsBytesTo.data(),
      MPI_BYTE,
      incomingFragments,
  ...
  CUDA_CALL(
      Malloc(&compositedColor, numPixelsOnThisRank * sizeof(*compositedColor)));
  ...
  if (rank == 0) {
    ...
    for (int node = 1; node < size; node++) {
      ...
      int end = std::min(
          numPixelsOrg, pixelEnd(node)); //(node+1)*fbSize.x*fbSize.y / size;
      if (begin < end)
    ...
    int end = std::min(
        numPixelsOrg, pixelEnd(rank)); //(node+1)*fbSize.x*fbSize.y / size;
    MPI_CALL(Send(compositedColor,
        (end - begin) /*numPixelsOnThisRankOrg*/
            * sizeof(*whereToWriteFinalPixels),
        MPI_BYTE,
        0,
        0,
        comm));
  ...
  if (g_pass++ == 0)
    printf(
        "rank %i: bytes in/out for counters: %.2fMB/out %.2fMB, frags: %.2fMB/%.2fMB, total: %.2fMB/%.2fMB \n",
        rank,
        ...
        float(stat_numBytesCountersOut + stat_numBytesFragsOut)
            / (1024 * 1024));
}
